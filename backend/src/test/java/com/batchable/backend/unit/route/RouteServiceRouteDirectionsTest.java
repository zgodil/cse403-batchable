...
/**
 * Unit tests for RouteService.getRouteDirections using a mocked GoogleRoutesClient.
 *
 * Verifies that the service correctly constructs the request object, delegates to the client, and
...
class RouteServiceRouteDirectionsTest {
  ...
  /**
   * Tests that getRouteDirections builds the correct RouteDirectionsRequest (origin, destination,
   * intermediates, travel mode) and passes it to the client. Also verifies that the response from
  ...
  void testGetRouteDirections() {
    ...
    when(mockClient.getRouteDirections(any(RouteDirectionsRequest.class), eq(false)))
        .thenReturn(fakeResponse);

    // Call the method under test
    RouteDirectionsResponse response =
        routeService.getRouteDirections(restaurantAddress, stops, false);

    // Verify the client was called with the correct data
    verify(mockClient)
        .getRouteDirections(argThat(req -> req.getOrigin().getAddress().equals(restaurantAddress)
            ...
            && req.getIntermediates().get(2).getAddress().equals("Stop 3")), eq(false));
