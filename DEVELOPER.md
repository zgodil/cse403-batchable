...
Put the add your .env file in the root of the project with your Google, Twilio API keys, Database URL, and any Auth0 credentials. The frontend currently reads `VITE_AUTH0_DOMAIN`, `VITE_AUTH0_CLIENT_ID`, and `VITE_AUTH0_AUDIENCE` from the environment, while the backend uses `AUTH0_ISSUER_URI` for the JWT issuer (defaults to the development tenant). **Do not deploy with the hard‑coded development tenant (`dev-3qj8cn8ady6wwlng.us.auth0.com`); exchange these for production values before going live.**
