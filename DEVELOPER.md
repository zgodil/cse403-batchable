...
## Process
First, ensure that the prerequisite software mentioned above is installed on your system. Then ensure that Docker is currently running: running `docker` in a terminal should indicate this.

If this is your first time building the project, place your `vars.env` file in the root of the project. **Important: do not place your environment variables in the file called `.env`. To succesfullly complete setup, both `.env` and `vars.env` should be present in the project's root directory**.
